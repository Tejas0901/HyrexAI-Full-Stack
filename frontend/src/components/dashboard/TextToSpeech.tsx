...
import { ttsApi, type TTSGeneration } from "@/services/tts";

const VOICES = [
  { id: "nisha", name: "Nisha", accent: "Indian", gender: "Female", initials: "NI" },
];
...
const TextToSpeech = () => {
  ...
  const handleGenerate = async () => {
    if (!text.trim() || !selectedVoice || phase === "generating") return;
    setPhase("generating");
    setIsPlaying(false);
    setError(null);
    setGeneration(null);
    ...
    try {
      ...
      const url = ttsApi.getDownloadUrl(result.id);
  ...
  const handleDownload = () => {
    ...
    const url = ttsApi.getDownloadUrl(generation.id);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          Convert text to natural-sounding audio with emotion control.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {VOICES.map((v) => (
                <button
                  key={v.id}
                  onClick={() =>
                    setSelectedVoice(selectedVoice === v.id ? null : v.id)
                  }
          ...
          {/* Emotions — only visible when voice is selected */}
          {selectedVoice && (
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 shadow-sm animate-in fade-in slide-in-from-top-2 duration-200">
              <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3">
                Emotion
              </h2>
          ...
          <button
            ...
            disabled={phase === "generating" || !text.trim() || !selectedVoice}
